...
class Contacts extends Admin_Controller {
	...
	function form() {
		...
		if (!$this->mdl_contacts->validate()) {
			...
			$this->redir->redirect('suppliers/details/supplier_id/' . uri_assoc('supplier_id', 4));
	...
	function delete() {
		...
		if (uri_assoc('contact_id', 4) and uri_assoc('supplier_id', 4)) {

			$this->mdl_contacts->delete(uri_assoc('supplier_id', 4), uri_assoc('contact_id', 4));
