...
class Mdl_Items extends MY_Model {
	...
	public function __construct() {
		...
		$this->table_name = 'mcb_bill_items';

		$this->primary_key = 'mcb_bill_items.bill_item_id';

		$this->select_fields = "
		SQL_CALC_FOUND_ROWS *,
        mcb_bill_items.tax_rate_id AS item_tax_rate_id,
        mcb_inventory.tax_rate_id AS inventory_tax_rate_id";

        $this->joins = array(
            'mcb_inventory' =>  array(
                'mcb_inventory.inventory_id = mcb_bill_items.inventory_id', 'left'
            )
        );
	...
	public function db_array() {
		...
		$db_array['bill_id'] = uri_assoc('bill_id', 4);
	...
	public function save($db_array, $bill_item_id = NULL) {
		...
		parent::save($db_array, $bill_item_id);
    ...
    public function save_order($bill_item_id, $item_order) {

        $this->db->where('bill_item_id', $bill_item_id);

        $this->db->set('item_order', $item_order);

        $this->db->update('mcb_bill_items');

    }

	public function delete($bill_item_id) {

		$this->db->where('bill_item_id', $bill_item_id);

		$this->db->delete(array('mcb_bill_items', 'mcb_bill_item_amounts'));
